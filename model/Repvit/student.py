...
from SPEMix.model.network.SPBlock import SPBlock
...
class RepViT(nn.Module):
    def __init__(self, cfgs,index1,index2,lambda1,lambda2, num_classes=4, distillation=False):
        ...
        self.SP=SPBlock(512,4)
    def get_mask_loss(self):
        return self.DAMix.mask_loss(self.mask1, self.lamda1)
    def forward(self, x):
        result={}
        x = self.features[0](x) # embed
        x = self.features[1](x)
        x = self.features[2](x)
        x = self.features[3](x)
        # self.mask1 = self.MP(x, self.lamda1, self.index1, 4)['mask']
        # self.mask2 = self.MP(x, self.lamda2, self.index2, 4)['mask']
        x = self.features[4](x)
